# Lesson 2: Jupyter Foundations

This folder contains the OSC-delivery files for Lesson 2.

## Files
- `notebook.ipynb`: Guided practice notebook for Jupyter notebook anatomy.
- `examples/arcade-leaderboard-notebook.ipynb`: Example workflow shown in notebook style.
- `examples/arcade-leaderboard-dense-script.py`: Equivalent example shown as a dense Python script.

## Recommended Use Order
1. Start with the Lesson 2 lesson plan.
2. Run `notebook.ipynb` with students.
3. Use the `examples/` files to compare notebook-first workflow vs dense script workflow.
4. Use the Lesson 2 student handout for consolidation and reflection.

## Notes
- For classroom reliability, keep notebook data access local when possible.
- Keep this folder aligned with lesson sequencing and update links when files move.
